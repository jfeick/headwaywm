...
{
    struct headway_output *output = wl_container_of(listener, output, destroy);
...
{
    struct headway_output *output = wl_container_of(listener, output, frame);
    struct headway_server * server = output->server;
...
{
    struct headway_server *server = wl_container_of(listener, server, new_output);
    struct wlr_output *wlr_output = data;
    ...
    struct  headway_output *output = calloc(1, sizeof(struct headway_output));
